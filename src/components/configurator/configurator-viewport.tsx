...
import { Camera, DoorOpen, Check, Maximize2 } from "lucide-react";
...
export function ConfiguratorViewport() {
  ...
  return (
    ...
    >
      {/* Door controls — top left */}
      <div className="absolute top-4 left-4 z-20 flex items-center space-x-2">
        <button
          onClick={() => toggleDoor("L")}
          className={cn(
            "px-3 py-1.5 text-sm font-mono border transition-all flex items-center space-x-1.5 backdrop-blur-md cursor-pointer",
            state.doors.L
      ...
      {/* Camera presets + fullscreen — bottom bar */}
      <div className="absolute bottom-4 left-4 right-4 z-20 flex flex-wrap items-center justify-between gap-2 pointer-events-none">
        <div className="flex items-center space-x-1 sm:space-x-2 bg-background/90 border border-border p-1 backdrop-blur-md pointer-events-auto overflow-x-auto max-w-full">
          <div className="px-2 py-1 text-xs text-muted-foreground uppercase hidden sm:flex items-center space-x-1">
          ...
          {CAMERA_PRESETS.map((preset) => {
            ...
            return (
              <button
                ...
                className={cn(
                  "px-2.5 py-1 text-sm font-mono transition-all whitespace-nowrap cursor-pointer flex items-center space-x-1",
                  isSelected
